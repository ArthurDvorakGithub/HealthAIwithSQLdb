...
namespace HealthAI.Areas.Identity.Pages.Account
{
    [AllowAnonymous]
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        //пользователь вошел и пытается создать нового пользователя
                        await _roleManager.CreateAsync(new IdentityRole(WC.AdminRole));
                    }
                    else
                    {
                        await _roleManager.CreateAsync(new IdentityRole(WC.PatientRole));
                        //  по дефолту новый пользователь - пациент, роль меняет админ
